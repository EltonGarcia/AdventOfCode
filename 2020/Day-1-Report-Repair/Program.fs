...
//PART 1:
let rec findExpenses list =
    match list with
    | x::xs -> if expensesDic.ContainsKey(expectedSum - x) then Some (x, expectedSum - x) else findExpenses xs
    | [] -> None

match findExpenses expenses with
    | Some (a, b) -> printfn "%d, %d = %d" a b (a*b)
    | None -> printfn "not found"

//PART 2:
for a in expenses do
    for b in expenses do
        let complement = expectedSum - (a + b)
        if expensesDic.ContainsKey(complement) then
                printfn "%d, %d, %d = %d" a b (complement) (a*b*complement)